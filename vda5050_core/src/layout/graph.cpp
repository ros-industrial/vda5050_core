...
// TODO(step4): replace these manual deep-equality helpers with the default
// operator== on LIF struct types once that lands.

bool nodes_equal(const Node& a, const Node& b)
{
  if (a.node_id != b.node_id) return false;
  if (a.map_id != b.map_id) return false;
  if (a.node_position.x != b.node_position.x) return false;
  if (a.node_position.y != b.node_position.y) return false;
  if (a.node_name != b.node_name) return false;
  if (a.node_description != b.node_description) return false;
  // vehicle_type_node_properties not deeply compared in V0; full deep compare
  // arrives in step 4 via operator==.
  if (
    a.vehicle_type_node_properties.size() !=
    b.vehicle_type_node_properties.size())
    return false;
  return true;
}
...
bool edges_equal(const Edge& a, const Edge& b)
{
  ...
  if (a.start_node_id != b.start_node_id) return false;
  if (a.end_node_id != b.end_node_id) return false;
  if (a.edge_name != b.edge_name) return false;
  if (a.edge_description != b.edge_description) return false;
  if (
    a.vehicle_type_edge_properties.size() !=
    b.vehicle_type_edge_properties.size())
    return false;
  return true;
}

bool stations_equal(const Station& a, const Station& b)
{
  if (a.station_id != b.station_id) return false;
  if (a.interaction_node_ids != b.interaction_node_ids) return false;
  if (a.station_name != b.station_name) return false;
  if (a.station_description != b.station_description) return false;
  if (a.station_height != b.station_height) return false;
  return true;
}

bool layouts_equal(const Layout& a, const Layout& b)
{
  if (a.layout_id != b.layout_id) return false;
  if (a.layout_version != b.layout_version) return false;
  if (a.layout_name != b.layout_name) return false;
  if (a.layout_level_id != b.layout_level_id) return false;
  if (a.layout_description != b.layout_description) return false;
  if (a.nodes.size() != b.nodes.size()) return false;
  if (a.edges.size() != b.edges.size()) return false;
  if (a.stations.size() != b.stations.size()) return false;
  ...
  {
    if (!edges_equal(a.edges[i], b.edges[i])) return false;
  }
  for (std::size_t i = 0; i < a.stations.size(); ++i)
  {
    if (!stations_equal(a.stations[i], b.stations[i])) return false;
  }
  return true;
}
...
{
  // TODO(step4): replace with lif_ == rhs.lif_ once LIF types get operator==.
  if (
    lif_.meta_information.project_identification !=
    rhs.lif_.meta_information.project_identification)
    return false;
  if (lif_.meta_information.creator != rhs.lif_.meta_information.creator)
    return false;
  if (
    lif_.meta_information.export_timestamp !=
    rhs.lif_.meta_information.export_timestamp)
    return false;
  if (
    lif_.meta_information.lif_version != rhs.lif_.meta_information.lif_version)
    return false;
  if (lif_.layouts.size() != rhs.lif_.layouts.size()) return false;
  for (std::size_t i = 0; i < lif_.layouts.size(); ++i)
  {
    if (!layouts_equal(lif_.layouts[i], rhs.lif_.layouts[i])) return false;
  }
  return true;
}
