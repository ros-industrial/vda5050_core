...
protected:
  ...
  /// create a valid new order that the vehicle can reach from the
  /// fully_released_order
  vda5050_types::Edge e6{"edge6",      6,
                         "node5",      "node7",
                         true,         {},
                         std::nullopt, std::nullopt,
                         std::nullopt, std::nullopt,
  ...
  vda5050_types::Edge unreleased_e4{"edge4",      4,
                                    "node1",      "node5",
                                    false,        {},
                                    std::nullopt, std::nullopt,
                                    std::nullopt, std::nullopt,
  ...
  vda5050_core::client::order::OrderManager orderManager{};

  /// Snapshot of the AGV's state if it has no existing order
  vda5050_types::State init_state{};

  /// Snapshot of the AGV's state after accepting and completing
  /// fully_released_order
  vda5050_types::State fully_released_state{};

  /// Snapshot of the AGV's state after accepting and completing
  /// partially_released_order
  ...
  void SetUp() override
  {
    ...
    /// NOTE: fully_released_state is AFTER the vehicle has completed the order,
    /// so node and edge states should be empty!
  ...
  std::vector<vda5050_types::NodeState> create_node_states(
  ...
  std::vector<vda5050_types::EdgeState> create_edge_states(
...
/// \brief Test if OrderManager successfully parses a new, valid order while no
/// current order exists on the vehicle. Assumes that vehicle is ready to
/// receive a new order.
TEST_F(OrderManagerTest, NewOrderNoCurrentOrder)
{
  bool is_new_order_accepted{
    orderManager.make_new_order(fully_released_order, init_state).valid};

  EXPECT_EQ(is_new_order_accepted, true);
}

/// \brief Test if OrderManager successfully parses a new, valid order while a
/// current order exists on the vehicle. Assumes that vehicle is ready to
/// receive a new order.
TEST_F(OrderManagerTest, NewOrderWithCurrentOrder)
{
  ...
  bool is_second_order_accepted{
    orderManager.make_new_order(order2, fully_released_state).valid};

  EXPECT_EQ(is_second_order_accepted, true);
}

/// \brief Test if OrderManager rejects an order if vehicle is still executing
/// an order
TEST_F(OrderManagerTest, NewOrderNodeStatesNotEmpty)
{
  ...
  std::vector<vda5050_types::NodeState> fully_released_node_states{
    create_node_states(unexecuted_nodes)};
  std::vector<vda5050_types::EdgeState> fully_released_edge_states{
    create_edge_states(unexecuted_edges)};
...
/// \brief Test if OrderManager rejects order and throws an error if vehicle
/// still has a horizon and is waiting on an update
TEST_F(OrderManagerTest, NewOrderVehicleWaitingForUpdate)
{
  bool is_first_order_accepted{
    orderManager.make_new_order(partially_released_order, init_state).valid};
...
/// \brief Test if OrderManager rejects order and throws an error if the new
/// order's first node is not trivially reachable by the vehicle
TEST_F(OrderManagerTest, NewOrderNodeNotTriviallyReachable)
{
  ...
  bool is_unreachable_order_accepted{
...
/// \brief Test if OrderManager successfully parses a valid order update when
/// the vehicle is ready for one
TEST_F(OrderManagerTest, BasicOrderUpdate)
{
  ...
  bool is_order_update_accepted =
    orderManager.update_current_order(order_update, partially_released_state)
      .valid;

  EXPECT_EQ(is_order_update_accepted, true);
}

/// \brief Test if OrderManager rejects order and throws an error if the order
...
TEST_F(OrderManagerTest, OrderUpdateDeprecated)
{
  ...
  bool is_deprecated_update_accepted =
    orderManager
      .update_current_order(deprecated_update_order, order_update_state)
      .valid;
...
/// \brief Test if OrderManager discards the order update if it is already on
/// the vehicle
TEST_F(OrderManagerTest, OrderUpdateOnVehicle)
{
  ...
  bool is_duplicate_order_update_accepted =
...
/// \brief Test if OrderManager rejects order and throws an error if the update
/// order is not a valid continuation of a previous order
TEST_F(OrderManagerTest, OrderUpdateInvalidContinuationOfCurrentOrder)
{
  ...
  /// Create an orderUpdate that is not a valid continuation from
  /// partially_released_order
  ...
  bool is_invalid_order_update_accepted =
    orderManager
      .update_current_order(invalid_continuation, partially_released_state)
      .valid;
...
/// \brief Test if OrderManager returns the graph elements from the base of an
/// order correctly
TEST_F(OrderManagerTest, GetNextGraphElement)
{
  ...
  if (
    std::optional<std::variant<vda5050_types::Node, vda5050_types::Edge>>
      graph_element_ref = orderManager.next_graph_element())
  {
    std::variant<vda5050_types::Node, vda5050_types::Edge> graph_element =
      graph_element_ref.value();
    vda5050_types::Node graph_element_1 =
      std::get<vda5050_types::Node>(graph_element);
  ...
  if (auto graph_element_ref = orderManager.next_graph_element())
  {
    ...
    vda5050_types::Edge graph_element_2 =
      std::get<vda5050_types::Edge>(graph_element);
  ...
  {
    auto graph_element = graph_element_ref.value();
    vda5050_types::Node graph_element_3 =
      std::get<vda5050_types::Node>(graph_element);
