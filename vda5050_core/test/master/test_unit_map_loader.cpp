...
TEST(MapLoaderTest, LoadFromFile_MalformedJson_ReturnsParseError)
{
  const std::string path = std::string(testing::TempDir()) +
                           "/vda5050_malformed_map_" +
                           std::to_string(::getpid()) + ".json";
  {
    std::ofstream out(path);
    out << R"({ "map_info": { "map_id": )";  // truncated, invalid JSON
  }
...
TEST(MapLoaderTest, LoadFromJson_EdgeMissingStartNode_ReturnsMissingField)
{
  auto j = minimal_valid_json();
  j["edges"][0].erase("start_node_id");
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::MISSING_REQUIRED_FIELD));
}

TEST(MapLoaderTest, LoadFromJson_NodeOnlyMap_NoEdgesKey_Succeeds)
{
  auto j = minimal_valid_json();
  j.erase("edges");
  auto r = load_from_json(j);
  ASSERT_TRUE(static_cast<bool>(r));
  EXPECT_TRUE(r.map->edges.empty());
}
...
TEST(MapLoaderTest, MapStatusUnknownValue_ReturnsInvalidFieldValue)
{
  auto j = minimal_valid_json();
  j["map_info"]["map_status"] = "DISABLE";  // typo for DISABLED
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::INVALID_FIELD_VALUE));
}

TEST(MapLoaderTest, MapStatusNotString_ReturnsInvalidFieldValue)
{
  auto j = minimal_valid_json();
  j["map_info"]["map_status"] = 1;
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::INVALID_FIELD_VALUE));
}

TEST(MapLoaderTest, MapStatusNull_ReturnsInvalidFieldValue)
{
  auto j = minimal_valid_json();
  j["map_info"]["map_status"] = nullptr;
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::INVALID_FIELD_VALUE));
}

TEST(MapLoaderTest, MapStatusEmptyString_ReturnsInvalidFieldValue)
{
  auto j = minimal_valid_json();
  j["map_info"]["map_status"] = "";
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::INVALID_FIELD_VALUE));
}

TEST(MapLoaderTest, MapStatusWrongCase_ReturnsInvalidFieldValue)
{
  auto j = minimal_valid_json();
  j["map_info"]["map_status"] = "enabled";  // matching is case-sensitive
  auto r = load_from_json(j);
  EXPECT_FALSE(static_cast<bool>(r));
  EXPECT_TRUE(has_error_of_type(r, MapLoadErrorType::INVALID_FIELD_VALUE));
}
