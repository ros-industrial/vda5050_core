...
public:
  ...
  ///
  /// \param sm Reference to the StateManager tracking the vehicle's current
  /// state.
  OrderManager();
  ...
  ///
  /// \return True if order update has been accepted by the order manager, false
  /// otherwise.
  ValidationResult update_current_order(
  ...
  /// \param state A snapshot of the vehicle's current state
  ///
  /// \return True if the new order has been accepted by the order manager,
  /// false otherwise
  ValidationResult make_new_order(
    vda5050_types::Order& order, const vda5050_types::State& state);

  /// \brief Returns the next graph element of the current order that is to be
  /// executed.
  ///
...
private:
  ...
  {
  public:
    explicit Order(vda5050_types::Order& order);
    ...
    const std::vector<std::variant<vda5050_types::Node, vda5050_types::Edge>>&
    graph() const
    {
      return graph_;
    }
    ...
    /// \brief Stitch this order with another order and update the
    /// order_update_id if stitching is successful.
    ///
  ...
  private:
    ...
    /// \brief The base of this order. Contains the released nodes and edges
    /// sorted in ascending sequenceId
    std::vector<std::variant<vda5050_types::Node, vda5050_types::Edge>> base_;

    /// \brief The horizion of this order. Contains the unreleased nodes and
    /// edges sorted in ascending sequenceId
    std::vector<std::variant<vda5050_types::Node, vda5050_types::Edge>>
      horizon_;

    /// \brief The last node in this order's base, or the last released node
    /// according to sequenceId
  ...
  /// \brief The index of current order's graph element that is to be dispatched
  /// next
  size_t current_graph_element_index_;

  /// TODO Should we be doing JSON validation here?
  /// \brief Reference to the JSON validator
  bool json_validator_;  /// TODO: (shawnkchan) I assume this needs to be
                         /// modular, but using this as a placeholder for now
  ...
  /// \brief Checks that vehicle is not waiting for an update to its current
  /// order (i.e: the vehicle's current order has no horizon)
  ///
  /// \return True if vehicle is waiting for an update to its current order
  bool is_vehicle_waiting_for_update();

  /// \brief Checks if order_update is a valid continuation from the current
  ...
  /// TODO This is currently incomplete. To find out what logic needs to be
  /// handled during order rejection
  void reject_order();

  /// \brief Helper function to return a vector of errors for an invalid
  /// continuation error during an order update.
  ///
  /// \param order_start_node_sequence_id sequenceId of the first node in the
  /// incoming order \param state_last_node_sequence_id lastNodeSequenceId of
  /// the vehicle's state \param order_start_node_node_id nodeId of the first
  /// node in the incoming order \param state_last_node_id nodeId of the
  /// vehicle's state
  ///
  /// \return Vector of Error structs for each error that occurred.
  std::vector<vda5050_types::Error> invalid_continuation_errors(
